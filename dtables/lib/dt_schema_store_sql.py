...
class DTSchemaStoreSQL:
    ...
    def get_schema(self, table_name, table_id=None):
        if table_id:
            dt_columns = []
            columns = self.session.query(Columns).filter_by(table_id=table_id).all()
            ...
            table_name = self.session.query(Users).filter_by(id=table_id).one().table_name
    ...
    def set_schema(self, dtable):
        if not dtable.table_id:
            ...
            if 'delete_column' in dtable.modifications:
                ...
                column = self.session.query(Columns).filter_by(id=column_id).one()
                self.delete_column(dtable, column)
            if 'delete_table' in dtable.modifications:
                table = self.session.query(Users).filter_by(id=dtable.table_id).one()
                self.delete(table)
    ...
    def add_column(self, dt_column):
        ...
        columns = self.session.query(Columns).filter_by(table_id=table_id).order_by(Columns.sequence).all()
        if not columns:
            sequence = 0
        else:
    ...
    def delete_column(self, dtable, column):
        columns = self.session.query(Columns).filter_by(table_id=dtable.table_id).order_by(Columns.sequence).all()
        ...
        if sequence < last_sequence:
            for column in columns[sequence+1:]:
                queried_column = self.session.query(Columns).\
    ...
    def update_column_sequence(self, dtable, column_id, new_sequence):
        ...
        columns = self.session.query(Columns).\
            filter_by(table_id=dtable.table_id).order_by(Columns.sequence).all()
        column_to_update = self.session.query(Columns).\
